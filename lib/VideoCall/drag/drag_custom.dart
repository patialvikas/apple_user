...
typedef DragUpdateCallback = void Function(Offset details);
...
class Draggable<T> extends StatefulWidget {
  ...
  /// `details` is whether the draggable [DragTarget] accepted it.
  ///
  ...
  /// recognizing a drag.
  @protected
  Future<MultiDragGestureRecognizer> createRecognizer(GestureMultiDragStartCallback onStart) async {
    switch (affinity) {
...
class LongPressDraggable<T> extends Draggable<T> {
  ...
  @override
  Future<DelayedMultiDragGestureRecognizer> createRecognizer(GestureMultiDragStartCallback onStart) async {
...
class _DraggableState<T> extends State<Draggable<T>> {
  ...
  void initState() {
    ...
    // _recognizer = widget.createRecognizer(_startDrag) as GestureRecognizer ;
  }
  ...
  Future<MultiDragGestureRecognizer>? _recognizer;
  int _activeCount = 0;
  ...
  _DragAvatar<T>? _startDrag(Offset position) {
    ...
    final _DragAvatar<T> avatar = _DragAvatar<T>(
      ...
      onDragUpdate: (Offset? dragOffset) {
        if(widget.onDragUpdate != null) widget.onDragUpdate!(dragOffset!);
      },
...
typedef _OnDragUpdate = void Function(Offset? offset);
...
class _DragAvatar<T> extends Drag {
  ...
  final _OnDragUpdate? onDragUpdate;
  ...
  void updateDrag(Offset globalPosition) {
    ...
    // Enter new targets.
    final _DragTargetState<T>? newTarget = targets.firstWhereOrNull(
